...
MitaHill Better Video 是一个面向本地 GPU 的视频处理平台，提供视频 / 图片超分增强、视频转换、字幕转录与翻译等功能。项目采用 **Flask + Vue + SQLite + Worker 子进程** 的结构，默认通过 Docker Compose 运行，适合部署在带 NVIDIA 显卡的个人服务器或工作站上。

项目的目标不是做成复杂的视频工厂，而是把常用的视频处理流程集中到一个稳定、清晰、可维护的 Web 页面里。

---------------

## 核心特性

- **视频与图片增强** - 基于 Real-ESRGAN 进行超分辨率增强，支持常用增强模型。
- **视频转换** - 提供常用视频封装、编码和水印能力，音频流默认保持复制，减少不必要的重编码。
- **转录与字幕** - 使用 OpenAI Whisper 进行语音识别，生成字幕、文本和带字幕视频。
- **翻译字幕** - 支持 OpenAI 兼容格式的翻译服务，适配本地 vLLM、Ollama 或兼容云服务。
- **GPU 编码检测** - 启动后按当前显卡和 FFmpeg 能力展示可用输出编码。
- **任务管理** - Web 页面查看任务状态、进度、日志，并支持删除任务实体文件和数据库记录。
- **管理中心** - 提供 GPU 监控、模型下载、转录 / 翻译测试、日志查看和运行配置入口。
- **容器化部署** - 默认使用 Docker Compose，运行数据集中保存在 `pre-run/storage/`。
...
- **显存**:
  - 4GB 显存可以运行较小 Whisper 模型和基础增强任务
  - 8GB 及以上体验更稳定
  - 大模型、长视频、高分辨率增强会明显增加显存压力
...
不要把常规运行数据写到仓库根目录。启动、重启、查看日志、排查运行状态，默认都在 `pre-run/` 下执行。
...
小型、稳定、反复使用的模型可以随镜像或项目持久化；大型模型不建议直接提交到源码仓库。

### 转录模型

转录使用原版 OpenAI Whisper，模型文件放在：

```text
pre-run/storage/models/transcription/whisper/
```

管理页面提供模型下载入口。任务创建页面只展示已安装模型，避免选择不存在的模型导致运行失败。

旧显卡运行建议：

- GTX 960 4GB 这类老卡优先使用 `small`
- `medium` 在 4GB 显存上可能 OOM，失败时应接受现实并换小模型
- CUDA 必须可用，项目不保留 CPU fallback 路径

---------------

## 翻译服务

翻译提供器仅保留 **OpenAI 兼容格式**。
...
如果本地部署模型，建议使用 vLLM 或 Ollama 暴露 OpenAI 兼容接口。这样项目侧只需要维护一套翻译调用逻辑，稳定性更高。
...
4GB 显存不保证能稳定运行 `medium`。如果 CUDA OOM，直接换 `small` 或更小模型，不建议通过复杂配置强行绕过。
...
如果这个项目对你有帮助，可以给一个 Star。

[![Star History Chart](https://api.star-history.com/svg?repos=MitaHill/mitahill-better-video&type=Date)](https://star-history.com/#MitaHill/mitahill-better-video&Date)
