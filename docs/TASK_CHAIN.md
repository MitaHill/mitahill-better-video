...
状态：**已实现，前端入口暂时隐藏，功能搁置**。代码、接口和测试都在，只是「链式」没有出现在页签里（`constants/workbench.js` 的 `CATEGORY_TABS` 里移除了该项），直接访问 `/chain` 仍可用于验证。要重新放出来，把那一项加回去即可。

本文描述在现有三条处理管线之上，「一次提交、多步串联」是怎么做的，以及为什么不照搬 ComfyUI 的节点图。
