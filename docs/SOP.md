...
- 主进程做的第一件事是打印启动横幅（`app/src/Utils/banner.py`），在初始化日志之前，所以横幅出现在所有日志行上方。横幅直接写 stdout 不走 logging：logging 会给每行加上时间戳和级别前缀把字元画打散，也会往日志库里塞几十行。Worker 和任务进程是独立进程，不打印横幅。
- Flask 提供 API 和静态前端。
- Worker 由主进程管理为独立子进程。
...
- 双语字幕用真实换行分行，译文在上、原文在下；字幕折行按 `max_line_chars` 逐行计算，两行互不影响，不把整段压成一行。
- 字幕封装使用 `/workspace/storage/tmp/subtitles/` 下的安全临时文件，ffmpeg 结束后立即删除。
- 视频增强合帧前逐个核对抽出来的每一帧都有对应输出，缺帧明确失败，不只比总数（`frames_out` 里的残留文件会把空洞补平）。断点续跑只信任连续的前缀，从第一个缺号处重新开始，不把历史空洞一路带到合帧。
- FFmpeg 编码器探测结果在进程内缓存，避免任务提交和合帧路径上反复 fork `ffmpeg -encoders`。编码器清单和单个编码器的试编码都只缓存成功结果：异常、空清单、试编码失败都不写缓存，下次重新探测。NVENC 会话占满、驱动抖动这类瞬时失败因此不会把整个进程永久卡在“无可用编码器”。
