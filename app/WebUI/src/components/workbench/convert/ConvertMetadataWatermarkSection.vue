<template>
  <div class="param-section" v-if="convertForm.convertMode === 'transcode'">
    ...
    <div class="inline-grid three">
      <div class="field compact"><label>标题</label><input v-model="convertForm.metaTitle" placeholder="可选" :disabled="isDisabled('metaTitle')" /></div>
      <div class="field compact"><label>作者</label><input v-model="convertForm.metaAuthor" placeholder="可选" :disabled="isDisabled('metaAuthor')" /></div>
      <div class="field compact"><label>注释</label><input v-model="convertForm.metaComment" placeholder="可选" :disabled="isDisabled('metaComment')" /></div>
    </div>
    <div class="inline-grid two">
      <label class="check-inline"><input type="checkbox" v-model="convertForm.watermarkEnableText" :disabled="isDisabled('watermarkEnableText')" />启用文字水印</label>
      <label class="check-inline"><input type="checkbox" v-model="convertForm.watermarkEnableImage" :disabled="isDisabled('watermarkEnableImage')" />启用图片水印</label>
    </div>
    <div class="field" v-if="convertForm.watermarkEnableText">
      <label>默认文字水印（当分段未填写文本时使用）</label>
      <input v-model="convertForm.watermarkDefaultText" placeholder="例如：MitaHill" :disabled="isDisabled('watermarkDefaultText')" />
    </div>
    <div class="inline-grid two">
      <div class="field compact"><label>默认透明度 (0.05~1)</label><input v-model.number="convertForm.watermarkAlpha" type="number" :min="numMin('watermarkAlpha', 0.05)" :max="numMax('watermarkAlpha', 1)" :step="numStep('watermarkAlpha', 0.05)" :disabled="isDisabled('watermarkAlpha')" /></div>
