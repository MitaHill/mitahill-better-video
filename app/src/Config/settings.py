...
def verify_models():
    sha_file = ROOT_DIR / "app/src/Data/models_sha256.yaml"
...
def get_env_preview_stride(key, default):
    val = os.getenv(key)
    if val is None:
        return default
...
try:
    ...
    TRANSCRIPTION_TRANSLATOR_PROMPT = os.getenv(
        ...
        "Translate to {{target_language}}. Use recent subtitle context. Put the translation in a code block.",
    ).strip()
except Exception as e:
