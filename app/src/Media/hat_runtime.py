"""HAT inference runtime.

The network definition is adapted from XPixelGroup/HAT (Apache-2.0).
Only the inference path used by this project is kept here.
"""
...
class OCAB(nn.Module):
    ...
    def forward(self, x, x_size, rpi):
        ...
        kv_windows = kv_windows.permute(1, 0, 5, 3, 4, 2).contiguous()
        kv_windows = kv_windows.view(2, -1, self.overlap_win_size * self.overlap_win_size, c)
...
class HATUpscaler:
    ...
    def __init__(self, model_path, tile=128, tile_pad=32, half=False, device=None, variant="real"):
    ...
    def _load_model(self):
        ...
        loadnet = torch.load(str(self.model_path), map_location=torch.device("cpu"))
        state = loadnet.get("params_ema") or loadnet.get("params") or loadnet
    ...
    def _run_tile(self, tensor):
        ...
        for y in range(tiles_y):
            for x in range(tiles_x):
                ...
                input_tile = tensor[:, :, input_start_y_pad:input_end_y_pad, input_start_x_pad:input_end_x_pad]
    ...
    def enhance(self, img_bgr, outscale=4):
        tensor = self._tensor_from_bgr(img_bgr)
        tensor, pad_h, pad_w = self._pad(tensor)
        output = self._run_tile(tensor) if self.tile_size > 0 else self.model(tensor)
        if pad_h or pad_w:
            _, _, h, w = output.shape
            output = output[:, :, : h - pad_h * self.scale, : w - pad_w * self.scale]
