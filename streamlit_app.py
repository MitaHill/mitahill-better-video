...
# --- Helper Utils ---
def get_client_ip():
    # Best effort to get client IP in Streamlit
    try:
        ...
        if not task:
            ...
            with st.expander("Task Details"):
                st.json({
                    ...
                    "Video Info": json.loads(task['video_info']),
                    "Parameters": json.loads(task['task_params'])
                })
            
            # Preview Images
            output_root = Path("/workspace/output")
            run_dir = output_root / f"run_{query_id}"
            ...
            if status == "COMPLETED":
                # Find output file
                # Simply list files in output_root starting with "sr_" and containing task filename stem
                # Or re-construct name from params. Simpler to check what worker produced.
                # In worker.py we logged "Finished. Output: xxx". Let's rely on finding file.
                msg = task['message']
                if "Output:" in msg:
